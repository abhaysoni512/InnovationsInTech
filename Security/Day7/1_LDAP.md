# Lightweight Directory Access Protocol (LDAP) :-
    ...
    LDAP is commonly used for authentication (e.g., single sign-on in enterprises), authorization, and storing organizational data. Popular implementations include OpenLDAP (open-source), Microsoft Active Directory (which supports LDAP), and others like Apache Directory Server.
...
# Features of LDAP:-
    1. LDAP is platform independent protocol.
    2. LDAP is used to access and maintain distributed directory information services over an IP network
    3. LDAP is based on client-server model.
    4. LDAP is used to provide central authentication for users meaning users can use same credentials to access multiple services.
    5. LDAP is used to store user information like username, password, email id etc
